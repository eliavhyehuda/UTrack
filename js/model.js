...
_.extend(ActivityStoreModel.prototype, {
    ...
    addActivityDataPoint: function(activityDataPoint) {
        this.activityArray.push(activityDataPoint);
        ...
        _.each(
            this.listeners,
            function (listener) {
                listener(ACTIVITY_DATA_ADDED_EVENT, new Date().toUTCString(), activityDataPoint);
            }
        );
    },
    ...
    removeActivityDataPoint: function(activityDataPoint) {
        var i = this.activityArray.indexOf(activityDataPoint);
        if (i >= 0) {
        ...
        _.each(
            this.listeners,
            function (listener) {
                listener(ACTIVITY_DATA_REMOVED_EVENT, new Date().toUTCString(), activityDataPoint);
            }
        );
    },
...
var GraphModel = function() {
    this.listeners = [];
    this.possibleGraph = ["none", "health", "time"];
    this.selectedGraph = "time";
...
_.extend(GraphModel.prototype, {
    ...
    selectGraph: function(graphName) {
        ...
        if (this.getAvailableGraphNames().indexOf(graphName) > -1 && graphName !== this.selectedGraph){
            _.each(
            this.listeners,
            function (listener) {
                listener(GRAPH_SELECTED_EVENT, new Date().toUTCString(), graphName);
            }
            );
